// campo de texto padrao para digitacao com aviso de erros
        ...
        <div className="w-full">
            ...
            <input
                className={`w-full bg-slate-50 border rounded-lg px-3 py-2 text-sm focus:outline-hidden focus:ring-2 focus:border-blue-500 focus:ring-blue-500/20 transition-all ${
                    erro ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : 'border-slate-200'
                } ${className}`}
                {...props}
            />
            {erro && <p className="text-xs text-red-600 font-medium mt-1">{erro}</p>}
        </div>
    );
};
